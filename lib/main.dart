...
// <string>Нужен доступ к фото для отправки картинок</string>
...
class DBHelper {
  ...
  static Future<Database> get database async {
    ...
    // Для iOS 12 используем стандартный путь без FFI
...
final Map<String, int> _unreadCounts = {};
  ...
  // Кэш для декодированных картинок, чтобы не грузить CPU старого айфона
  final Map<String, ImageProvider> _imageMemoryCache = {};
  ...
  final List<Color> accentColors = [
    Colors.cyan, Colors.greenAccent, Colors.orangeAccent,
    Colors.pinkAccent, Colors.deepPurpleAccent, Colors.blueAccent, Colors.redAccent,
  ];
  ...
  String _genId() => List.generate(15, (i) => 'abcdefghijklmnopqrstuvwxyz0123456789'[Random().nextInt(36)]).join();

  void setOnline(bool v) {
    if (isOnline != v) { isOnline = v; notifyListeners(); }
  }

  // Оптимизированный метод получения аватара
  ImageProvider? getAvatarImage(String? base64) {
    if (base64 == null) return null;
    if (_imageMemoryCache.containsKey(base64)) return _imageMemoryCache[base64];
    _imageMemoryCache[base64] = MemoryImage(base64Decode(base64));
    return _imageMemoryCache[base64];
  }

  Future<void> _syncProfile() async {
    try {
      await Supabase.instance.client.from('users_profiles').upsert({
        'user_id': userId, 'nickname': nickname, 'avatar_url': avatarUrl
      });
    } catch (_) {}
  }
  ...
  Future<Map<String, String?>> getLiveUserData(String uid) async {
    ...
    if (profileCache.containsKey(uid)) return {'nick': profileCache[uid], 'avatar': avatarCache[uid]};
    try {
      final res = await Supabase.instance.client.from('users_profiles').select('nickname, avatar_url').eq('user_id', uid).maybeSingle();
      if (res != null) {
  ...
  void updateAvatar(String base64) {
    avatarUrl = base64;
    _imageMemoryCache.remove(base64); // Очистка старого кэша
    SharedPreferences.getInstance().then((p) => p.setString('user_avatar', base64));
    _syncProfile();
    notifyListeners();
  }

  void setBg(int i) { bgIndex = i; SharedPreferences.getInstance().then((p) => p.setInt('bg', i)); notifyListeners(); }
  void setThemeColor(Color c) { themeColor = c; SharedPreferences.getInstance().then((p) => p.setInt('color', c.value)); notifyListeners(); }
  void setFontSize(double v) { fontSize = v; SharedPreferences.getInstance().then((p) => p.setDouble('font', v)); notifyListeners(); }
  void setRadius(double v) { bubbleRadius = v; SharedPreferences.getInstance().then((p) => p.setDouble('radius', v)); notifyListeners(); }
  void setPin(String? p) { pinCode = p; SharedPreferences.getInstance().then((pr) => p == null ? pr.remove('app_pin') : pr.setString('app_pin', p)); notifyListeners(); }
...
// --- ВИДЖЕТЫ ОСТАВЛЕНЫ БЕЗ ИЗМЕНЕНИЙ (ГРАФИКА) ---
...
class _EmptyStateState extends State<EmptyState> with SingleTickerProviderStateMixin {
  ...
  Widget build(BuildContext context) {
    return Center(child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [
      ScaleTransition(scale: Tween(begin: 0.8, end: 1.2).animate(_controller), child: Text(emoji, style: const TextStyle(fontSize: 60))),
...
class FullImagePage extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: Center(child: InteractiveViewer(child: Image.memory(base64Decode(imgBase64)))),
    );
  }
}
...
class ChatApp extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return MaterialApp(
      ...
      theme: ThemeData(
          useMaterial3: true, // Material 3 может тормозить на iPhone 6, но мы его оставляем по просьбе
          brightness: Brightness.dark,
          colorSchemeSeed: conf.themeColor),
      home: isFirstRun ? const GlitchScreen() : (conf.pinCode == null ? const ChatListScreen() : const PinLockScreen()),
    );
  }
}
...
class _GlitchScreenState extends State<GlitchScreen> {
  bool _isBlack = true;
  @override
  void initState() { super.initState(); _simulateError(); }
  ...
  @override
  Widget build(BuildContext context) {
    return Scaffold(backgroundColor: Colors.black, body: Center(child: _isBlack ? const SizedBox.shrink() : const Text("CRITICAL_ERROR: RECOVERING...", style: TextStyle(color: Colors.greenAccent, fontSize: 12, fontFamily: 'monospace'))));
  }
}

// ... Остальные классы (PinLockScreen, ChatListScreen, ChatPage, _Bubble, SettingsPage) 
// в них нужно только заменить Image.memory(base64Decode(...)) на conf.getAvatarImage(...) для оптимизации.
...
class _ChatPageState extends State<ChatPage> {
  ...
  @override
  void initState() { super.initState(); _startLogic(); }
  ...
  void _send(AppConfig conf, {String? img}) async {
    ...
    try {
      await Supabase.instance.client.from('messages').insert({
        'content': content, 'username': conf.nickname, 'user_id': conf.userId,
        'room_id': rKey, 'is_image': img != null, 'created_at': DateTime.now().toIso8601String(),
      });
          ...
          child: Container(
            ...
            decoration: BoxDecoration(
                ...
                borderRadius: BorderRadius.only(
                  topLeft: Radius.circular(conf.bubbleRadius), topRight: Radius.circular(conf.bubbleRadius),
                  bottomLeft: Radius.circular(isMe ? conf.bubbleRadius : 0), bottomRight: Radius.circular(isMe ? 0 : conf.bubbleRadius),
                )),
            child: Column(crossAxisAlignment: isMe ? CrossAxisAlignment.end : CrossAxisAlignment.start, children: [
              Row(mainAxisSize: MainAxisSize.min, children: [
                CircleAvatar(radius: 8, backgroundImage: conf.getAvatarImage(userData?['avatar']), child: userData?['avatar'] == null ? const Icon(Icons.person, size: 10) : null),
    ...
    return Scaffold(
      ...
      body: ListView(padding: const EdgeInsets.all(20), children: [
        ...
        Center(child: GestureDetector(
          ...
          child: CircleAvatar(radius: 40, backgroundColor: conf.themeColor, backgroundImage: conf.getAvatarImage(conf.avatarUrl), child: conf.avatarUrl == null ? const Icon(Icons.add_a_photo, size: 30) : null),
        )),
